function calcularLagra2() {
    ...
    //((((x-x1)/(x0-x1))*((x-x1)/(x0-x2)))*(fx0))+((((x-x0)/(x1-x0))*((x-x2)/(x1-x2)))*(fx1))+((((x-x0)/(x2-x0))*((x-x1)/(x2-x1)))*(fx2))
...
function calcularErrorActivate() {
    document.getElementById('check').onchange = function() {
        ...
        document.getElementById('res1').disabled = !this.checked;
    };
...
function calcularErrorL() {
    ...
    var resFinal=parseFloat(document.getElementById('res1').value);

    errorP = ((resFinal-ep)/ep)*100;
